    ...
    $.widget("ui.tagit", {
        ...
        _create:function () {
            ...
            if (this.options.select) {
                ...
                this.select.data('chosen', {}); //Hack to disallow chosen jquery extension
...
function tagit_init() {
    this.each( function () {
        ...
        var defaults = { select : true, sortable : true, triggerKeys: ['enter', 'comma', 'tab', 'semicolon'] };
        defaults.tagSource = that.data('values');
        if (that.data('placeholder')) defaults.placeholder = that.data('placeholder');
        if (that.data('ajax')) {
            defaults.tagSource = function(request, callback) {

                request.exist = new Array();

                jQuery.each(this.element.closest('ul').tagit('tags'), function(i, e) {
                    request.exist.push(e.value);
                });
        ...
        var data = jQuery.extend( defaults, that.data('init'));
        that.tagit(data);
    });
}

jQuery(document).on('onFFLoad', function(e, parent) {
    tagit_init.call(jQuery(parent).find('.tagit'));
});
